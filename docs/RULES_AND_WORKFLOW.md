...
**Не реализовано.** §8.1–8.3 реализованы (см. ниже); это единственный пункт раздела 8, оставшийся в backlog.

### 8.5 Как это реализовано на самом деле

Алгоритм §8.2 требует видеть **все** мутации от **всех** подходящих правил одновременно, прежде чем что-либо применять — а значит, правила одного события нельзя выполнять как N независимых Celery-задач (было так изначально): Celery не гарантирует порядок исполнения задач, и приоритет учитывался бы только при постановке в очередь, а не при заверешении.

Вместо этого `RuleService.evaluate_rules_for_event` ([backend/app/services/rules.py](../backend/app/services/rules.py)) диспетчерит **одну** задачу `execute_rules_batch` ([backend/app/worker/tasks/sandbox.py](../backend/app/worker/tasks/sandbox.py)) на событие, с полным списком подходящих правил, отсортированным по `priority` asc. Внутри задачи `run_rules_batch` ([backend/app/engine/interpreter.py](../backend/app/engine/interpreter.py)) — чистая функция без побочных эффектов, реализующая ровно алгоритм §8.2 (плюс: список **всех** проигравших на поле, не только первого) — применяет правила последовательно в одной структуре `ctx.record`, откатывая проигравшую мутацию сразу после обнаружения конфликта, чтобы более поздние (по приоритету) правила видели победившее значение в своих условиях.

Результат сохраняется одной транзакцией: смёрженные мутации записи, побочные действия (create/update/delete/notify/webhook) от всех сработавших правил, и — для журнала — по одной строке `RuleExecutionLog` на правило плюс одна строка `logic.rule_conflict_log` на каждое поле с конфликтом (не на каждого проигравшего). API: `GET /apps/{app_id}/rules/conflicts`.
